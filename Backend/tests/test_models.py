...
class TestModelUpload:
    ...
    def test_upload_model_success(
        self, client: TestClient, auth_headers: dict, temp_model_file: str
    ):
    ...
    def test_upload_model_versioning(
        self, client: TestClient, auth_headers: dict, temp_model_file: str
    ):
        ...
        with open(temp_model_file, "rb") as f:
            response1 = client.post(
                "/api/v1/models/upload",
                headers=auth_headers,
                data={"name": "versioned_model", "model_type": "sklearn"},
                files={"file": ("model.pkl", f, "application/octet-stream")},
            )
...
class TestModelListing:
    ...
    def test_list_models_with_data(
        self, client: TestClient, auth_headers: dict, test_model: Model
    ):
...
class TestModelDetails:
    ...
    def test_get_model_not_found(self, client: TestClient, auth_headers: dict):
        ...
        response = client.get(
            "/api/v1/models/00000000-0000-0000-0000-000000000000", headers=auth_headers
        )
...
class TestModelDeletion:
    ...
    def test_delete_model(
        ...
        response = client.delete(
            f"/api/v1/models/{test_model.id}", headers=auth_headers
        )
...
class TestModelAnalytics:
    ...
    def test_get_analytics(
        ...
        response = client.get(
            f"/api/v1/models/{test_model.id}/analytics", headers=auth_headers
        )
    ...
    def test_get_analytics_custom_period(
        ...
        response = client.get(
            f"/api/v1/models/{test_model.id}/analytics?days=30", headers=auth_headers
        )
