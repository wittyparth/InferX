...
def test_rate_limit_headers():
    ...
    print(
        f"X-RateLimit-Remaining: {response.headers.get('X-RateLimit-Remaining', 'Not set')}"
    )
...
def test_update_api_key():
    ...
    response = requests.patch(
        ...
        headers=headers,
        json={"name": "Updated Test Key", "is_active": True},
    )
...
def test_revoke_api_key():
    ...
    response = requests.post(
        f"{BASE_URL}/api-keys",
        headers=headers,
        json={"name": "Temporary Key for Deletion", "expires_days": 1},
    )
